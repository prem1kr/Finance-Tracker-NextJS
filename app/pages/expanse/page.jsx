  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-white dark:bg-gray-800 p-3 rounded-lg shadow-lg border border-gray-200 dark:border-gray-600 transition-colors duration-200">
        <p className="font-semibold text-gray-900 dark:text-white">{payload[0].payload.category || payload[0].payload.title}</p>
    ...
    <div className="px-1 sm:px-3 pt-10 bg-white dark:bg-gray-900 w-full min-h-screen transition-colors duration-200 -mt-3 ">
      <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-md mb-6 transition-colors duration-200 max-w-full mx-auto">
        <div className="flex justify-between items-center">
          <h2 className="text-xl font-semibold text-gray-900 dark:text-white transition-colors duration-200">Expense Overview</h2>
          <div className="flex space-x-2">
            <button onClick={() => setShowAddModal(true)} className="bg-purple-600 text-white rounded px-4 py-2 hover:bg-purple-700 transition cursor-pointer">
              + Add Expense
            </button>
          </div>
        </div>
        <div className="mt-4 h-56 sm:h-80">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData} margin={{ top: 20, right: 10, left: 10, bottom: 10 }}>
              ...
              <YAxis domain={[0, "dataMax + 50"]} tick={{ fill: theme === "dark" ? "#94a3b8" : "#6b7280", fontSize: 10 }} axisLine={false} tickLine={false} />
      ...
      <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-md max-w-full mx-auto transition-colors duration-200">
        ...
        <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {expenses.map((expense) => (
            <li
              key={expense._id}
              className="group relative flex items-center justify-between bg-gray-50 dark:bg-gray-700 rounded p-3 shadow hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors duration-200 "
            >
              <div className="flex items-center space-x-4">
                <span className="w-11 h-11 flex items-center justify-center rounded-full bg-gray-100 dark:bg-gray-800 text-xl">{expense.icon ?? "❓"}</span>
                <div>
                  <div className="font-semibold text-gray-900 dark:text-white">{expense.category || expense.title}</div>
                  <div className="text-xs text-gray-500 dark:text-gray-400">{new Date(expense.date).toLocaleDateString()}</div>
                </div>
              </div>
      ...
      {showAddModal && <AddExpense onClose={() => setShowAddModal(false)} onSuccess={handleAddSuccess} />}
